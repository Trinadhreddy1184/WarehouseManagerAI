...
class ProductLookupAgent:
    ...
    def _load_prompt_template(self):
        ...
        return PromptTemplate(
            template=template, input_variables=["input", "chat_history","tools"]
        )
    ...
    def lookup(self, query: str, chat_history: List[Dict[str, Any]] = []) -> str:
        try:
            formatted_input = self.prompt_template.format_prompt(input=query, chat_history=chat_history, tools=self.tools_for_agent)
